...
import XP_LETTER_LINE from '../../assets/banner_letters/usables_letter/line/image';

import "./letters.css";

interface FirstSectionProps { innerPosition: any }

const FirstSection: React.FC<FirstSectionProps> = ({ innerPosition }) => {

    const amorSize = useResponsiveSize(5245, 5245, 2173, false);
    const arteSize = useResponsiveSize(5293, 5293, 2594, false);
    const designSize = useResponsiveSize(5305, 5305, 2138, false);
    const kitsSize = useResponsiveSize(3788, 3788, 2881, false);
    const usablesSize = useResponsiveSize(3586, 3586, 3325, false);
    const xpSize = useResponsiveSize(5279, 5279, 2042, false);
                ...
                <div className="w-full h-full relative m-auto mt-[6rem]">
                    <div className="w-full h-full left-0 right-0 m-auto absolute text-center min-w-[1250px] max-w-[1250px] transition easy-in-out duration-700">
                        ...
                        <h1 className={contentText === "ARTE" ? 'opacity-100 transition-every' : 'opacity-0 transition-every'}>
                            <LoadImage alt="Arte Banner Text" w={arteSize.sizeResponsive.width} h={arteSize.sizeResponsive.height}
                                       instantLoad={true} image={ARTE_LETTER} />
                        </h1>

                        <h1 className={contentText === "ARTE" ? 'opacity-100 text-stroke transition-every' : 'opacity-0 text-stroke transition-every'}>
                            <LoadImage alt="Arte Banner Line Text" w={arteSize.sizeResponsive.width} h={arteSize.sizeResponsive.height}
                                       instantLoad={true} image={ARTE_LETTER_LINE} />
                        </h1>


                        <h1 className={contentText === "AMOR" ? 'opacity-100 transition-every' : 'opacity-0 transition-every'}>
                            <LoadImage alt="Amor Banner Text" w={amorSize.sizeResponsive.width} h={amorSize.sizeResponsive.height}
                                       instantLoad={true} image={AMOR_LETTER} />
                        </h1>

                        <h1 className={contentText === "AMOR" ? 'opacity-100 transition-every text-stroke' : 'opacity-0 transition-every text-stroke'}>
                            <LoadImage alt="Amor Banner Line Text" w={amorSize.sizeResponsive.width} h={amorSize.sizeResponsive.height}
                                       instantLoad={true} image={AMOR_LETTER_LINE} />
                        </h1>

                        <h1 className={contentText === "KITS" ? 'opacity-100 transition-every px-[10rem]' : 'opacity-0 transition-every px-[10rem]'}>
                            <LoadImage alt="Kits Banner Text" w={kitsSize.sizeResponsive.width} h={kitsSize.sizeResponsive.height}
                                       instantLoad={true} image={KITS_LETTER} />
                        </h1>
                        ...
                        <h1 className={contentText === "KITS" ? 'opacity-100 transition-every text-stroke px-[10rem]' : 'opacity-0 transition-every text-stroke px-[10rem]'}>
                            <LoadImage alt="Kits Banner Text" w={kitsSize.sizeResponsive.width} h={kitsSize.sizeResponsive.height}
                                       instantLoad={true} image={KITS_LETTER_LINE} />
                        </h1>

                        <h1 className={contentText === "DISEÑO" ? 'opacity-100 transition-every' : 'opacity-0 transition-every'}>
                            <LoadImage alt="Design Banner Text" w={designSize.sizeResponsive.width} h={designSize.sizeResponsive.height}
                                       instantLoad={true} image={DESIGN_LETTER} />
                        </h1>
                        <h1 className={contentText === "DISEÑO" ? 'opacity-100 transition-every text-stroke' : 'opacity-0 transition-every text-stroke'}>
                            <LoadImage alt="Design Banner Text" w={designSize.sizeResponsive.width} h={designSize.sizeResponsive.height}
                                       instantLoad={true} image={DESIGN_LETTER_LINE} />
                        </h1>

                        <h1 className={contentText === "USABLES" ? 'opacity-100 transition-every px-[10rem]' : 'opacity-0 transition-every px-[10rem]'}>
                            <LoadImage alt="Usables Banner Text" w={usablesSize.sizeResponsive.width} h={usablesSize.sizeResponsive.height}
                                       instantLoad={true} image={USABLES_LETTER} />
                        </h1>
